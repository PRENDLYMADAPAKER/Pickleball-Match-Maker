...
async function saveToStorage() {
  ...
  // Guard: If session is empty or reset, clear local storage instead of writing empty data
...
function updateMetrics() {
  ...
  const hasActiveSession = sessionData && (sessionData.players?.length > 0 || sessionData.title);
  if (elActive) elActive.textContent = hasActiveSession ? "1" : "0";

  const runs = parseInt(localStorage.getItem(SESSIONS_RUN_KEY) || '0', 10);
  if (elRun) elRun.textContent = runs;

  const clubs = getClubs();
  if (elClubs) elClubs.textContent = clubs.length;
}
...
async function endSession() {
  ...
  // 1. Increment total sessions run counter
  let currentRuns = parseInt(localStorage.getItem(SESSIONS_RUN_KEY) || '0', 10);
  currentRuns += 1;
  localStorage.setItem(SESSIONS_RUN_KEY, currentRuns.toString());

  // 2. Render Leaderboard BEFORE clearing in-memory sessionData
  renderFinalLeaderboard();
  showView('viewLeaderboard');

  // 3. Clear cloud session from backend API
  const sessionId = localStorage.getItem(SESSION_ID_KEY);
  if (sessionId) {
    try {
      await fetch(`/api/session?sessionId=${encodeURIComponent(sessionId)}`, {
...
function renderFinalLeaderboard() {
  ...
  // Render Complete Standings Table
