...
/* ==========================================
   CLUB MANAGEMENT (CREATE, EDIT, SEARCH, DELETE)
   ========================================== */
...
function openClubModal(clubId = null) {
  const email = localStorage.getItem(USER_EMAIL_KEY);
  if (!email) {
    alert("You must be logged in to manage clubs.");
    toggleAuthDrawer();
    return;
  }
  ...
  if (clubId) {
    ...
    if (!club) return;
    if (modalTitle) modalTitle.textContent = "Edit Pickleball Club";
    if (saveClubBtn) saveClubBtn.textContent = "Update Club";

    document.getElementById('clubNameInput').value = club.name || '';
    document.getElementById('clubStartedInput').value = club.started !== 'N/A' ? club.started : '';
    document.getElementById('clubOrganizerNameInput').value = club.organizer || '';
    document.getElementById('clubOrganizerDetailsInput').value = club.organizerDetails !== 'Organizer' ? club.organizerDetails : '';
    document.getElementById('clubSocialLinkInput').value = club.socialLink !== '#' ? club.socialLink : '';
    document.getElementById('clubContactDetailsInput').value = club.contact !== 'N/A' ? club.contact : '';
    document.getElementById('clubAreaScopeInput').value = club.scope !== 'General' ? club.scope : '';
    document.getElementById('clubMembersInput').value = club.members ? club.members.join('\n') : '';
  } else {
    if (modalTitle) modalTitle.textContent = "Create New Pickleball Club";
    if (saveClubBtn) saveClubBtn.textContent = "Register Club";

    document.getElementById('clubNameInput').value = '';
    document.getElementById('clubStartedInput').value = '';
    document.getElementById('clubOrganizerNameInput').value = '';
    document.getElementById('clubOrganizerDetailsInput').value = '';
    document.getElementById('clubSocialLinkInput').value = '';
    document.getElementById('clubContactDetailsInput').value = '';
    document.getElementById('clubAreaScopeInput').value = '';
    document.getElementById('clubMembersInput').value = '';
  }
...
async function saveClub() {
  ...
  if (!userEmail) {
    alert("Logged in user required to manage a club.");
    return;
  }
  ...
  const clubData = {
    id: currentEditingClubId || ('club_' + Date.now()),
    name,
    started: started || 'N/A',
    organizer,
  ...
  closeClubModal();
  await fetchMetricsAndClubs();
  alert(currentEditingClubId ? "Club updated successfully!" : "Pickleball Club registered successfully!");
  currentEditingClubId = null;
}
...
async function deleteClub(clubId) {
  ...
  try {
    ...
    console.error("Failed to delete club from cloud:", err);
  }
...
function renderClubs() {
  ...
  const searchQuery = (document.getElementById('clubSearchInput')?.value || '').toLowerCase().trim();
  let clubs = getClubs();

  if (searchQuery) {
    clubs = clubs.filter(c => 
      (c.name && c.name.toLowerCase().includes(searchQuery)) ||
      (c.organizer && c.organizer.toLowerCase().includes(searchQuery)) ||
  ...
  if (clubs.length === 0) {
    container.innerHTML = `<p style="color:var(--text-muted); font-size: 0.8rem; padding: 10px 0;">${searchQuery ? 'No clubs found matching your search.' : 'No pickleball clubs registered yet. Create one today!'}</p>`;
    return;
  }
  ...
  clubs.forEach(club => {
    const isOwner = userEmail && club.createdBy === userEmail;
    ...
    card.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom: 6px;">
        ...
        <div style="display: flex; gap: 6px; align-items: center;">
          ...
          ${isOwner ? `
            <button type="button" class="btn-secondary btn-compact" style="padding: 2px 6px; font-size: 0.65rem;" onclick="openClubModal('${club.id}')">Edit</button>
            <button type="button" class="btn-danger" style="padding: 2px 6px; font-size: 0.65rem;" onclick="deleteClub('${club.id}')">Delete</button>
          ` : ''}
        </div>
      </div>
      ...
      <div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed var(--border-color);">
        <p style="font-size: 0.72rem; font-weight: 700; color: var(--text-muted);">TOTAL MEMBERS: <span style="color: var(--accent);">${club.members ? club.members.length : 0}/50</span></p>
      </div>
    `;
