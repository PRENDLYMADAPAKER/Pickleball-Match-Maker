...
const SESSIONS_RUN_KEY = 'pb_sessions_run_count';
...
/* ==========================================
   REAL-TIME FIREBASE METRICS SYNC
   ========================================== */

function startMetricsSync() {
  syncMetricsFromFirebase();
  if (metricsPollInterval) clearInterval(metricsPollInterval);
  metricsPollInterval = setInterval(syncMetricsFromFirebase, 4000);
}

async function syncMetricsFromFirebase() {
  try {
    // 1. Sync Active Sessions
    const activeRes = await fetch('/api/session?sessionId=global_active_sessions');
    if (activeRes.ok) {
      const data = await activeRes.json();
      const activeMap = data?.sessionData || {};
      const now = Date.now();
      let count = 0;
      Object.keys(activeMap).forEach(key => {
        if (activeMap[key] && (now - (activeMap[key].updatedAt || 0) < 10 * 60 * 1000)) {
          count++;
        }
      });
      liveMetrics.activeSessionsCount = count;
    }

    // 2. Sync Total Sessions Run
    const statsRes = await fetch('/api/session?sessionId=global_stats');
    if (statsRes.ok) {
      const data = await statsRes.json();
      if (data?.sessionData && typeof data.sessionData.totalSessionsRun === 'number') {
        liveMetrics.totalSessionsRun = data.sessionData.totalSessionsRun;
        localStorage.setItem(SESSIONS_RUN_KEY, liveMetrics.totalSessionsRun.toString());
      }
    }

    // 3. Sync Clubs
    const clubsRes = await fetch('/api/session?sessionId=global_clubs');
    if (clubsRes.ok) {
      const data = await clubsRes.json();
      let clubs = [];
      if (Array.isArray(data?.sessionData)) {
        clubs = data.sessionData;
      } else if (data?.sessionData?.clubs && Array.isArray(data.sessionData.clubs)) {
        clubs = data.sessionData.clubs;
      }
      if (clubs.length > 0 || localStorage.getItem(CLUB_STORAGE_KEY)) {
        localStorage.setItem(CLUB_STORAGE_KEY, JSON.stringify(clubs));
        liveMetrics.totalClubs = clubs.length;
        renderClubs();
      }
    }
    ...
    console.error("Error syncing Firebase metrics:", err);
  }
}

async function updateActiveSessionStatus(sessionId, isActive) {
  if (!sessionId || sessionId.startsWith('global_')) return;
  try {
    const res = await fetch('/api/session?sessionId=global_active_sessions');
    let currentMap = {};
    if (res.ok) {
      const data = await res.json();
      if (data?.sessionData && typeof data.sessionData === 'object') {
        currentMap = data.sessionData;
      }
    }
    ...
    Object.keys(currentMap).forEach(key => {
      if (currentMap[key] && currentMap[key].updatedAt && (now - currentMap[key].updatedAt < 10 * 60 * 1000)) {
        cleanedMap[key] = currentMap[key];
      }
    });

    if (isActive) {
      cleanedMap[sessionId] = { updatedAt: now };
    } else {
    ...
    await fetch('/api/session', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionId: 'global_active_sessions',
        sessionData: cleanedMap
      })
    });
  } catch (err) {
    console.error("Error updating active session status:", err);
  }
}

async function recordSessionCompleted() {
  try {
    const res = await fetch('/api/session?sessionId=global_stats');
    let total = 0;
    if (res.ok) {
      const data = await res.json();
      if (data?.sessionData && typeof data.sessionData.totalSessionsRun === 'number') {
        total = data.sessionData.totalSessionsRun;
      } else {
        total = parseInt(localStorage.getItem(SESSIONS_RUN_KEY) || '0', 10);
      }
    }
    total += 1;
    localStorage.setItem(SESSIONS_RUN_KEY, total.toString());

    await fetch('/api/session', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionId: 'global_stats',
        sessionData: { totalSessionsRun: total }
      })
    });
  } catch (err) {
...
async function saveClubToFirebase(newClub) {
  try {
    const res = await fetch('/api/session?sessionId=global_clubs');
    let clubs = [];
    if (res.ok) {
      const data = await res.json();
      if (Array.isArray(data?.sessionData)) {
    ...
    await fetch('/api/session', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionId: 'global_clubs',
        sessionData: clubs
      })
    });
  } catch (err) {
    console.error("Error saving club to Firebase:", err);
  }
}
...
async function loginOrRegister() {
  ...
  try {
    ...
    if (hasCloudSession) {
      resumeSession();
    } else if (sessionData && (sessionData.players?.length > 0 || sessionData.title)) {
      await saveToStorage();
      resumeSession();
    }
...
async function saveToStorage() {
  ...
  if (!sessionData.title && (!sessionData.players || sessionData.players.length === 0)) {
    localStorage.removeItem(STORAGE_KEY);
    if (sessionId) {
      await updateActiveSessionStatus(sessionId, false);
    }
    return;
  }
  ...
  if (sessionId) {
    try {
      ...
      await updateActiveSessionStatus(sessionId, true);
    } catch (err) {
...
async function saveClub() {
  ...
  await saveClubToFirebase(newClub);
...
/* ==========================================
   METRICS COUNTER & END SESSION LEADERBOARD
   ========================================== */

function updateMetrics() {
  const elActive = document.getElementById('metricActiveSessions');
  const elRun = document.getElementById('metricSessionsRun');
  const elClubs = document.getElementById('metricTotalClubs');

  const localHasActive = sessionData && (sessionData.players?.length > 0 || sessionData.title);
  const displayActive = Math.max(liveMetrics.activeSessionsCount, localHasActive ? 1 : 0);
  
  const localRuns = parseInt(localStorage.getItem(SESSIONS_RUN_KEY) || '0', 10);
  const displayRuns = Math.max(liveMetrics.totalSessionsRun, localRuns);

  const localClubs = getClubs();
  const displayClubs = Math.max(liveMetrics.totalClubs, localClubs.length);

  if (elActive) elActive.textContent = displayActive.toString();
  if (elRun) elRun.textContent = displayRuns.toString();
  if (elClubs) elClubs.textContent = displayClubs.toString();
}
...
async function endSession() {
  ...
  // 1. Record session completed in Firebase
  await recordSessionCompleted();

  // 2. Render Leaderboard BEFORE clearing in-memory sessionData
  renderFinalLeaderboard();
  showView('viewLeaderboard');

  // 3. Clear cloud session from backend API and unregister active session
  const sessionId = localStorage.getItem(SESSION_ID_KEY);
  if (sessionId) {
    try {
      await fetch(`/api/session?sessionId=${encodeURIComponent(sessionId)}`, {
      ...
      console.error("Error clearing cloud session on end:", err);
    }
  }

  // 4. Clear local storage and reset sessionData state in memory
  localStorage.removeItem(STORAGE_KEY);
  sessionData = { 
    title: "", 
    mode: "doubles", 
